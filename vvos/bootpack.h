...
extern void load_gdtr(int limit, int addr); 		/*set gdtr*/
...
void boxfill8(unsigned char *vran, int xsize, unsigned char c, int x0, int y0, int x1, int y1);
...
struct MEMMAN
{
	int frees, maxfrees, lostsize, losts; 					// frees、losts表示个数，不是索引，注意边界问题
...
struct SHTCTL
{
	...
	int xsize, ysize, top; 					// top表示索引，不是个数，注意边界问题
