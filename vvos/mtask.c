...
struct TASK *task_init(struct MEMMAN *memman)
{
	...
	for (i = 0; i < MAX_TASKS; ++i)
	{
		...
		set_segmdesc(gdt + TASK_GDT0 + i, 104 - 1, (unsigned int)&taskctl->task0[i].tss, AR_TSS32);
	}
	task = task_alloc();
	task->flags = TASK_FLAGS_RUNNING;
	task->priority = 2;
...
{
	struct TASKLEVEL *tl = &taskctl->level[taskctl->now_lv];
	struct TASK *new_task, *old_task = tl->tasks[tl->now];
	++tl->now;
	if(tl->now == tl->runnings)
	{
		tl->now = 0;
